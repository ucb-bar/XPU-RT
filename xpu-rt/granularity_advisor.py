...
# duration is under this fraction of the tightest periodic period -- i.e.
# there's enough slack that going coarser (fewer, bigger dispatches --
...
def analyze_granularity(records: list[DispatchRecord]) -> list[GranularityAdvice]:
    """The core comparison: non-periodic dispatch duration vs. periodic period.

    For each non-periodic job (see `group_by_periodicity`), compares its
    worst-case (max) dispatch duration against the tightest (minimum) period
    among periodic jobs in this schedule:
      - duration exceeds the tightest period -> "finer" (this dispatch
        would overrun that periodic job's cadence if they ever shared a
        core)
      - duration is under `_COARSER_SLACK_RATIO` of it -> "coarser" (safe
        to use fewer, bigger dispatches -- less scheduling overhead)
      - otherwise -> "unchanged"
    ...
    tightest_base = min(periodic_periods, key=lambda b: periodic_periods[b])
    ...
    for base_id, group in non_periodic.items():
        ...
        if max_duration > tightest_period:
            recommended = "finer"
            reason = (
                f"the desired granularity for {base_id}'s dispatches should be "
                f"finer: its largest dispatch ({max_duration:.2f} ms) exceeds "
                f"{tightest_base}'s period ({tightest_period:.2f} ms) -- it "
                f"would overrun {tightest_base}'s cadence if they shared a core"
            )
        elif max_duration < tightest_period * _COARSER_SLACK_RATIO:
            recommended = "coarser"
            reason = (
                f"{base_id}'s largest dispatch ({max_duration:.2f} ms) is well "
                f"under {int(_COARSER_SLACK_RATIO * 100)}% of {tightest_base}'s "
                f"period ({tightest_period:.2f} ms) -- fewer, bigger dispatches "
                f"would cut scheduling overhead without risking {tightest_base}'s deadline"
            )
        else:
            recommended = "unchanged"
            reason = (
                f"{base_id}'s largest dispatch ({max_duration:.2f} ms) fits "
                f"comfortably within {tightest_base}'s period ({tightest_period:.2f} ms)"
            )
