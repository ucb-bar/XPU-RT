...
raises :class:`~xpu_rt.runtime.errors.UnsupportedTopologyError`
...
from xpu_rt.runtime.errors import UnsupportedTopologyError
...
class _BlobRef:
    name: str
    offset_floats: int  # into the float32 view of xpu_rt_model_blob
    num_floats: int
...
class LoweredModel:
    ...
    Attributes:
        ...
        forward_c_source: The model-specific straight-line forward C.
            Depends on the XPU-RT foundational runtime's headers
            (``xpu_rt/ops.h``, ``xpu_rt/types.h``) and links against
            ``libxpu_rt_runtime.a`` (see ``runtime/Makefile``).
...
def lower_cnn_to_c(
    ...
    *,
    ...
    lines: list[str] = [
        ...
        "/* Auto-generated by xpu_rt.runtime.embedded.cnn_lowering. */",
        ...
        "#include <string.h>",
        "",
        "extern const uint8_t xpu_rt_model_blob[];",
        "",
        "static inline const float *blob_f32(size_t offset_floats) {",
        "    return (const float *)xpu_rt_model_blob + offset_floats;",
        "}",
        "",
        f"/* Arena layout: 4 × {buf_bytes}-byte buffers (A, B, R, S). */",
        f"#define XPU_RT_BUF_BYTES {buf_bytes}",
        "",
        "cg_status_t xpu_rt_model_forward(const float *input_f32,",
        "                                  float *output_f32,",
        ...
        "    float *A = (float *)(base + 0 * XPU_RT_BUF_BYTES);",
        "    float *B = (float *)(base + 1 * XPU_RT_BUF_BYTES);",
        "    float *R = (float *)(base + 2 * XPU_RT_BUF_BYTES);",
        "    float *S = (float *)(base + 3 * XPU_RT_BUF_BYTES);",
