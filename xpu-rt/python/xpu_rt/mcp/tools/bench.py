    ...
    if cached is None and target and op_family:
        ...
        if rec is not None:
            cached = BenchResultEntry(
                perf_us=rec.perf_us,
                correct=rec.correctness_passed,
...
def register_bench_result(
    ...
    *,
    ...
    if pending.target and pending.op_family:
        shared_db().record_kernel_perf(
            KernelPerfRecord(
                target=pending.target,
                op_family=pending.op_family,
...
def lookup_bench_result(
    ...
    *,
    ...
    if entry is None:
        return {"ok": True, "session_id": session_id, "found": False, "fingerprint": fp}
    return {
        "ok": True,
        "session_id": session_id,
        "found": True,
...
class McpBenchFn:
    ...
    def __call__(self, contract, codegen_result):  # noqa: ANN001
        ...
        shape_sig = ";".join(",".join(str(d) for d in t.shape.dims) for t in contract.io.inputs)
        dtype_sig = ";".join("/".join(t.dtype_class) for t in contract.io.inputs)
        ...
        return BenchResult(perf_us=None, correct=True, notes="bench queued via MCP")
...
BENCH_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Fulfill a pending bench request with the measured perf and correctness. Persists to the on-disk KernelDB."
        ),
        "phase": "transform",
