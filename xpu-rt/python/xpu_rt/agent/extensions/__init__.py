"""Local (user-authored) LLM extensions for XPU-RT.

A user who installs XPU-RT via pip can drop custom ``Tool`` /
``InventSlot`` definitions in ``~/.xpu_rt/extensions/*.py``. On the
first call to :func:`xpu_rt.llm.registry.get_registry`, every file is
...
``xpu_rt.extensions`` under the installed package is reserved for
