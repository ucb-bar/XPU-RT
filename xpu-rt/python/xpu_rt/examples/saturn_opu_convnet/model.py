"""ConvNet demo for the Saturn OPU Zephyr/FireSim bring-up.

Architecture is a 1:1 copy of Merlin's ``opu_bench_convnet`` (ResNet-style
CNN with 3 stages). Self-contained — no imports beyond PyTorch.

Exposes:

* :func:`build_model` — eval-mode :class:`ConvNet` with deterministic params.
* :func:`default_inputs` — canonical input tuple with a fixed RNG seed so
  goldens stay reproducible across runs.
