...
def list_extensions(root: Path | None = None) -> list[ContribExtension]:
    ...
    Scans ``root`` (defaults to ``~/.xpu_rt/extensions``), builds a
...
def status(root: Path | None = None) -> dict[str, Any]:
    """Roll-up for ``xpu_rt contrib status``."""
...
Generated by ``xpu_rt.contrib.draft``. Locks the copied invent-slot
...
def test_{slot_name}_module_imports() -> None:
    ...
    mod = importlib.import_module(
        "xpu_rt.agent.invent_slots.contrib.{slot_name}"
    )
...
def test_{slot_name}_registration_roundtrips() -> None:
    from xpu_rt.llm.registry import Registry
    from xpu_rt.agent.extensions.local_loader import _import_file
    from pathlib import Path
    import xpu_rt.agent.invent_slots.contrib.{slot_name} as mod
    src = Path(mod.__file__)
    reg = Registry()
    module = _import_file(src)
    # Collect declared tools / slots via the loader's helper.
    from xpu_rt.agent.extensions.local_loader import _register_from_module
...
def draft_pr(
    ...
    *,
    ...
    into ``repo_root/python/xpu_rt/agent/invent_slots/contrib/<slot>.py``,
    ...
    upstream_dir = repo / "python" / "xpu-rt" / "agent" / "invent_slots" / "contrib"
    ...
    if commit and not result.errors:
        ...
        msg = (
            ...
            f"Auto-generated by xpu_rt contrib draft.\n"
    ...
    result.gh_command = (
        ...
        f"--body 'Auto-drafted by xpu_rt contrib draft; see commit for details.'"
    )
