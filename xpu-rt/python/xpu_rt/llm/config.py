...
def resolve_llm_selection(
    ...
    *,
    ...
    source = "cli" if provider or model or record is not None or record_dir is not None else (
        "env" if os.environ.get("COMPGEN_LLM_BACKEND") or os.environ.get("COMPGEN_LLM_MODEL") else "auto"
    )
    ...
    selected_record = record if record is not None else os.environ.get("COMPGEN_LLM_NO_RECORD", "") not in {
        "1", "true", "TRUE", "yes", "YES",
    }
    selected_record_dir = Path(
        record_dir or os.environ.get("COMPGEN_LLM_RECORD_DIR") or ".compgen_cache/llm_logs"
    )
    return LLMSelection(
...
def selection_status(selection: LLMSelection) -> dict[str, str]:
    ...
    if selection.provider == "gemini":
        available = any(env_values.get(name) or os.environ.get(name) for name in (
