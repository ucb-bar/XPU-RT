...
class StageContractTestSuite:
    ...
    def test_input_contract_accepts_valid_ir(self) -> None:
        ...
        violations = self.stage.verify_contract(self.sample_module, self.stage.input_contract())
    ...
    def test_output_contract_holds_after_run(self) -> None:
        ...
        result = self.stage.run(self.sample_module.clone(), self.target, self.capabilities)
        assert result.passed, f"Stage failed: {result.contract_violations}"
        if result.module is not None:
            violations = self.stage.verify_contract(result.module, self.stage.output_contract())
    ...
    def test_stage_is_idempotent(self) -> None:
        ...
        result1 = self.stage.run(self.sample_module.clone(), self.target, self.capabilities)
        if result1.module is not None and result1.passed:
            result2 = self.stage.run(result1.module.clone(), self.target, self.capabilities)
