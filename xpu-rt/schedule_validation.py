...
# `overlap_fixer` and `count_overlaps` were byte-identical copies of
# `postprocessing`'s, which `packing.py` already imports from there. Two copies
# of the pass that decides whether a schedule double-books a core is one more
# than can be kept in agreement, so they are re-exported rather than repeated.
from postprocessing import count_overlaps, overlap_fixer  # noqa: F401
