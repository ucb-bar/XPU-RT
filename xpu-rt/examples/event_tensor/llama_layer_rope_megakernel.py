...
parity by adding two operators that  explicitly omitted:
    ...
    * **RoPE** (rotary position embedding) -- a per-(m_tile, head) device
      ...
      host-precomputed cos / sin tables.  Uses HF's "half-rotation"
      formula::

          q_rot = q * cos + rotate_half(q) * sin
          where rotate_half(x) = cat([-x[..., d/2:], x[..., :d/2]], dim=-1)

    * **Causal attention mask** -- ``compute_scores`` masks ``scores[i, j]``
...
Validated on real TinyLlama-1.1B layer-0 weights against an HF-faithful
PyTorch reference -- both the megakernel and the reference apply the
same RoPE half-rotation and the same causal mask.
"""
...
# ---------------------------------------------------------------------------
# Triton bodies (10 device functions; new entries: rope_apply + causal mask)
# ---------------------------------------------------------------------------
...
# RoPE: per (m_tile, h) rotates Q[h, m_rows, :] and K[h, m_rows, :] in place.
# Uses the HF "half-rotation" formula:
#   q_rot[..., :half]  = q[:half]  * cos[:half]  - q[half:] * sin[:half]
#   q_rot[..., half:]  = q[half:]  * cos[half:]  + q[:half] * sin[half:]
# (Same for K.)  The cos/sin tables are precomputed by the host and
# laid out as (S, D_HEAD); we read the rows for our m_tile.
_ROPE_BODY = r"""
m_tile = task_id // H_HEADS
h      = task_id %  H_HEADS

# Wait until ALL qkv_proj tasks have finished (Q and K must be present
# for every m_tile and every head before rotation can safely overwrite).
...
cos1_ptrs = COS_ptr + m_rows[:, None] * D_HEAD + d_first[None, :]
cos2_ptrs = COS_ptr + m_rows[:, None] * D_HEAD + d_second[None, :]
sin1_ptrs = SIN_ptr + m_rows[:, None] * D_HEAD + d_first[None, :]
sin2_ptrs = SIN_ptr + m_rows[:, None] * D_HEAD + d_second[None, :]
cos1 = tl.load(cos1_ptrs); cos2 = tl.load(cos2_ptrs)
sin1 = tl.load(sin1_ptrs); sin2 = tl.load(sin2_ptrs)
...
# Rotate K[h, m_rows, :]
k1_ptrs = K_ptr + h * (S * D_HEAD) + m_rows[:, None] * D_HEAD + d_first[None, :]
k2_ptrs = K_ptr + h * (S * D_HEAD) + m_rows[:, None] * D_HEAD + d_second[None, :]
...
# compute_scores with causal mask added.
_COMPUTE_SCORES_BODY = r"""
h        = task_id // Q_TILES
q_tile   = task_id %  Q_TILES

# Wait until ALL RoPE tasks have completed (so K is ready for every key
# position, not just our q_tile).
...
# Causal mask: positions assumed equal to row indices, so key j is valid
# for query i iff j <= i.  Mask the upper triangle with a large negative.
...
def build_decoder_layer_event_graph_with_rope(
    ...
    block.add_op(EventTensorOp.create(properties={
        ...
        "event_type": EventTensorTypeAttr([1]),
        "wait_count": IntegerAttr(n_qkv_total, IntegerType(64)),  # one rope task per qkv task
    }))
    block.add_op(EventTensorOp.create(properties={
    ...
    for fn, count in [
        ...
        ("rope_apply",      n_qkv_total),       # one rope task per (m_tile, h)
    ...
    total = (m_tiles + n_qkv_total + n_qkv_total + 2 * n_attn_tasks
             + m_tiles + m_tiles + 2 * n_gate_up + n_mlp_down)
    sm_count = max(1, min(total, 16))
    graph = GraphOp(
        sym_name="llama_layer_rope",
        policy="dynamic",
        sm_count=sm_count,
        body=Region([block]),
    )
...
def compile_llama_layer_rope(
    n_heads: int = 4, seq_len: int = 16, head_dim: int = 16,
    ...
    mod, graph = build_decoder_layer_event_graph_with_rope(
    ...
    spec = DynamicMegakernelLoweringSpec(
        ...
        constexpr_args=(
            "S", "D_HEAD", "D_HEAD_HALF", "D_HIDDEN", "I", "H_HEADS",
...
# ---------------------------------------------------------------------------
# RoPE table helper (matches HF Llama exactly)
# ---------------------------------------------------------------------------
...
def hf_rope_tables(
    ...
    """Compute cos / sin tables of shape (seq_len, head_dim) the way HF Llama does.

    HF concatenates ``[freqs, freqs]`` along the head_dim axis, so the
    cos/sin tables effectively repeat the per-half values twice.
    """
...
def run_llama_layer_rope(
    ...
    total_tasks = (
        m_tiles + n_qkv_total + n_qkv_total + 2 * n_attn_tasks + m_tiles + m_tiles
    ...
    compiled.kernel_callable[(compiled.sm_count,)](
        ...
        # constexprs
        S, D_HEAD, D_HEAD // 2, D_HIDDEN, I, H,
        q_tiles, i_tiles, n_tiles,
...
def _apply_rope_hf(
    ...
    """HF Llama half-rotation RoPE."""
    # q, k: (H, S, D_HEAD)
    # cos, sin: (S, D_HEAD)
    q_rot = q * cos[None, :, :] + _rotate_half(q) * sin[None, :, :]
    k_rot = k * cos[None, :, :] + _rotate_half(k) * sin[None, :, :]
    return q_rot, k_rot


def reference_llama_layer_rope(
    x: torch.Tensor,
    w_norm1: torch.Tensor, w_q: torch.Tensor, w_k: torch.Tensor, w_v: torch.Tensor,
    w_o: torch.Tensor,
    w_norm2: torch.Tensor,
    ...
    """HF-faithful Llama decoder-layer reference (RoPE + causal SDPA)."""
    S, D_HIDDEN = x.shape
    ...
    q, k = _apply_rope_hf(q, k, cos, sin)
...
__all__ = [
    "CompiledLlamaLayerRope",
    "build_decoder_layer_event_graph_with_rope",
...
if __name__ == "__main__":
    ...
    print(f"Emitted Llama layer (RoPE + causal): {compiled.kernel_name}")
    ...
    print("PASS: emitted Llama layer megakernel with RoPE + causal mask matches HF-faithful reference.")
