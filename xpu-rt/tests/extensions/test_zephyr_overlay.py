...
def _make_fake_bundle(root: Path) -> Path:
    ...
    bundle = root / "bundle"
    bundle.mkdir()
    (bundle / "libxpu_rt_model.a").write_bytes(b"!<arch>\n")  # valid ar header byte
    (bundle / "model_blob.c").write_text(
        "const unsigned char xpu_rt_model_blob[] = {0};\nconst unsigned xpu_rt_model_blob_size = 1;\n"
    )
    (bundle / "xpu_rt_model.h").write_text(
        "#pragma once\n"
        "int xpu_rt_init(void*, unsigned long);\n"
        "int xpu_rt_invoke(const void*, unsigned long, void*, unsigned long);\n"
        "void xpu_rt_shutdown(void);\n"
    )
    return bundle
...
def test_cmake_lists_links_xpu_rt_model(tmp_path: Path) -> None:
    bundle = _make_fake_bundle(tmp_path)
    zephyr = _make_fake_zephyr_root(tmp_path)
    result = emit_overlay(bundle, zephyr)
    cmake = result.paths.cmake_lists.read_text()
    assert "find_package(Zephyr" in cmake
    assert "xpu_rt_model" in cmake
    assert "libxpu_rt_model.a" in cmake
...
def test_main_c_exercises_xpu_rt_c_abi(tmp_path: Path) -> None:
    bundle = _make_fake_bundle(tmp_path)
    zephyr = _make_fake_zephyr_root(tmp_path)
    result = emit_overlay(bundle, zephyr)
    main = result.paths.main_c.read_text()
    for symbol in ["xpu_rt_init", "xpu_rt_invoke", "xpu_rt_shutdown", "xpu_rt_pal_log"]:
