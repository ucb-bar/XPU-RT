...
class TestToolDescriptors:
    def test_tools_in_registry(self) -> None:
        ...
        top-level ``xpu_rt.mcp.tools`` registry."""
        from xpu_rt.mcp.tools import TARGET_TOOLS, get_all_tools

        names = {t["name"] for t in TARGET_TOOLS}
        assert names == {
            "xpu_rt_list_targets",
            "xpu_rt_describe_target",
    ...
    def test_tool_handlers_callable(self) -> None:
        from xpu_rt.mcp.tools.targets import (
            xpu_rt_describe_target,
            xpu_rt_list_targets,
            xpu_rt_register_target,
        )

        assert callable(xpu_rt_list_targets)
        assert callable(xpu_rt_describe_target)
        assert callable(xpu_rt_register_target)
...
class TestListAndDescribe:
    def test_list_empty_registry(self) -> None:
        from xpu_rt.mcp.tools.targets import xpu_rt_list_targets

        out = xpu_rt_list_targets()
        assert out["count"] == 0
        assert out["tree"] == {}
        assert out["targets"] == []
    ...
    def test_describe_known_target(self) -> None:
        from xpu_rt.mcp.tools.targets import xpu_rt_describe_target
        from xpu_rt.targets.registry import register_target
        ...
        d = xpu_rt_describe_target(target_id="gpu.nvidia.blackwell")
        assert d["status"] == "ok"
    ...
    def test_describe_unknown_target_returns_status(self) -> None:
        ...
        d = xpu_rt_describe_target(target_id="gpu.tenstorrent.gridx")
...
class TestRegister:
    def test_register_with_no_adapters(self) -> None:
        ...
        from xpu_rt.mcp.tools.targets import xpu_rt_register_target

        out = xpu_rt_register_target(
            target_class="gpu",
            vendor="custom",
            arch="experimental",
...
class TestMcpDispatchConvention:
    """REQ-027: ``xpu_rt.mcp.server`` dispatches every handler as
    ...
    def test_list_targets_accepts_session_arg(self) -> None:
        ...
        out = xpu_rt_list_targets(_FakeSM())
        assert "count" in out

    def test_describe_target_accepts_session_arg(self) -> None:
        from xpu_rt.mcp.tools.targets import xpu_rt_describe_target

        out = xpu_rt_describe_target(_FakeSM(), target_id="x.y.z")
    ...
    def test_dispatch_via_tool_dict_does_not_raise(self) -> None:
        """Mirrors ``xpu_rt/mcp/server.py:107`` exactly:
        ...
        called = 0
        for tool in TARGET_TOOLS:
            if tool["name"] == "xpu_rt_list_targets":
                tool["handler"](sm)
                called += 1
            elif tool["name"] == "xpu_rt_describe_target":
                tool["handler"](sm, target_id="gpu.foo.bar")
                called += 1
            elif tool["name"] == "xpu_rt_register_target":
                tool["handler"](
                    sm,
                    target_class="gpu",
                    vendor="reg027",
...
class TestToolDictPhase:
    ...
    it satisfies the ``xpu_rt.plugins._validate_mcp_tool`` shape."""
    ...
    def test_validator_accepts_target_tools(self) -> None:
        """The pack-side ``xpu_rt.mcp.tools`` entry-point validator
