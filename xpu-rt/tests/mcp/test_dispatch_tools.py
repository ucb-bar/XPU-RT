...
def test_request_then_register_then_lookup_round_trip(session_manager) -> None:
    out = request_dispatch_decision(
        ...
        priors=[{"target": "cuda-a100", "granularity": "normal", "confidence": 0.7, "reason": "default"}],
    ...
    decision = {
        "per_target": {
            "cuda-a100": {"granularity": "normal", "rationale": "single-op default"},
        },
...
def test_mcp_dispatch_llm_returns_cached_decision(session_manager) -> None:
    ...
    contract = KernelContractV3(
        ...
        io=IOContract(
            inputs=(TensorIO(name="a", shape=ShapeClass(dims=(None,)), dtype_class=("f32",)),),
