"""Regression test for the ``mode="xpu_rt_ir"`` execution path.
...
def test_benchmark_xpu_rt_ir_runs_and_matches_eager() -> None:
    """``mode='xpu_rt_ir'`` produces the same output as ``mode='eager'``
    ...
    xpu_rt_result = executor.benchmark(
        model=fx.model,
        sample_inputs=fx.example_inputs,
        device="cpu",
        mode="xpu_rt_ir",
    ...
    assert isinstance(xpu_rt_result, BenchmarkResult)
    assert xpu_rt_result.mode == "xpu_rt_ir"
    assert xpu_rt_result.device == "cpu"
    assert xpu_rt_result.sample_output is not None

    # Shape parity.
    assert tuple(xpu_rt_result.sample_output.shape) == tuple(eager_result.sample_output.shape)

    # Numerical parity: the bridged module with no passes applied should
    # reproduce eager output exactly within fp32 tolerance.
    max_abs_diff = (xpu_rt_result.sample_output - eager_result.sample_output).abs().max().item()
    assert max_abs_diff < 1e-5, f"max_abs_diff={max_abs_diff} exceeds fp32 tolerance"


def test_benchmark_xpu_rt_ir_matches_eager_golden_output() -> None:
    """Sanity: xpu_rt_ir output also matches the fixture's precomputed
    ...
    assert xpu_rt_result.sample_output is not None
    assert tuple(xpu_rt_result.sample_output.shape) == tuple(fx.eager_output.shape)

    max_abs_diff = (xpu_rt_result.sample_output - fx.eager_output).abs().max().item()
    assert max_abs_diff < 1e-5, f"max_abs_diff={max_abs_diff} vs golden eager"


def test_benchmark_xpu_rt_ir_requires_payload_module_and_export() -> None:
...
def test_benchmark_xpu_rt_ir_on_cuda_device_falls_back_to_cpu() -> None:
    """``mode='xpu_rt_ir'`` with ``device='cuda'`` silently routes to
