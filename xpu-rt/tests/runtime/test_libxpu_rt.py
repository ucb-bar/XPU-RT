"""Integration tests driving libxpu_rt from Python.

Skipped when the native library is not built. To build it::

    /usr/bin/cmake -B runtime/native/libxpu_rt/build -S runtime/native/libxpu_rt
    /usr/bin/cmake --build runtime/native/libxpu_rt/build
...
import pytest
from xpu_rt.runtime.native.library import CgRtError, available

if not available():
    pytest.skip("libxpu_rt shared library not built", allow_module_level=True)

from xpu_rt.runtime.native.device import (  # noqa: E402
    Buffer,
    CommandBuffer,
    DeviceClass,
    EventDType,
