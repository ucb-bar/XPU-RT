...
def test_gqa_layer_megakernel_matches_gqa_reference(n_heads: int, n_kv_heads: int) -> None:
    ...
    compiled = compile_llama_layer_gqa(
        ...
        seq_len=S, head_dim=D_HEAD, intermediate_dim=I,
    )
    ...
    ref = reference_llama_layer_gqa(
        ...
        cos, sin, n_heads=n_heads, n_kv_heads=n_kv_heads, head_dim=D_HEAD,
    )
    err = (got - ref).abs().max().item()
    assert err < 5e-3, (
        f"GQA megakernel (H={n_heads}, N_KV={n_kv_heads}) diverges by {err}"
    )
...
def test_gqa_emitted_kernel_carries_kv_repeat_constexpr() -> None:
    ...
    compiled = compile_llama_layer_gqa(
        n_heads=4, n_kv_heads=2, seq_len=16, head_dim=16, intermediate_dim=32,
    )
