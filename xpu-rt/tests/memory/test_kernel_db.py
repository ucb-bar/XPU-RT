...
def test_record_then_best_returns_lowest_perf_us(db: KernelDB) -> None:
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="abc",
        perf_us=100.0, correctness_passed=True,
    ))
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="abc",
        perf_us=80.0, correctness_passed=True,
    ))
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="abc",
        perf_us=120.0, correctness_passed=True,
    ))
...
def test_best_ignores_correctness_failures(db: KernelDB) -> None:
    ...
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="abc",
        perf_us=30.0, correctness_passed=False,
    ))
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="abc",
        perf_us=60.0, correctness_passed=True,
    ))
    best = db.best_kernel_perf("cuda-a100", "matmul", "abc")
    assert best is not None
...
def test_list_kernel_perf_filters_by_target_and_op(db: KernelDB) -> None:
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="matmul", fingerprint="a",
        perf_us=10.0, correctness_passed=True,
    ))
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-a100", op_family="softmax", fingerprint="b",
        perf_us=20.0, correctness_passed=True,
    ))
    db.record_kernel_perf(KernelPerfRecord(
        target="cuda-h100", op_family="matmul", fingerprint="c",
        perf_us=5.0, correctness_passed=True,
    ))
...
def test_fusion_history_returns_records_sorted_by_time(db: KernelDB) -> None:
    t0 = time.time()
    db.record_fusion_decision(FusionDecisionRecord(
        target="cuda-a100", producer_role="matmul", consumer_role="silu",
        decision="fuse", predicted_speedup=1.5, observed_speedup=1.2,
        measured_at=t0,
    ))
    db.record_fusion_decision(FusionDecisionRecord(
        target="cuda-a100", producer_role="matmul", consumer_role="silu",
        decision="fuse", predicted_speedup=1.5, observed_speedup=0.9,
        measured_at=t0 + 100,
    ))
...
def test_average_observed_speedup_calibrates_predictions(db: KernelDB) -> None:
    ...
    for obs in (1.0, 1.2, 0.9, 1.1, 0.8):
        db.record_fusion_decision(FusionDecisionRecord(
            target="cuda-a100", producer_role="matmul", consumer_role="silu",
            decision="fuse", predicted_speedup=1.5, observed_speedup=obs,
        ))
...
def test_average_observed_speedup_is_none_when_only_null_observations(db: KernelDB) -> None:
    ...
    db.record_fusion_decision(FusionDecisionRecord(
        target="cuda-a100", producer_role="matmul", consumer_role="silu",
        decision="fuse", predicted_speedup=1.5, observed_speedup=None,
    ))
    avg = db.average_observed_speedup("cuda-a100", "matmul", "silu")
    assert avg is None
...
def test_average_speedup_isolates_by_target_and_pair(db: KernelDB) -> None:
    db.record_fusion_decision(FusionDecisionRecord(
        target="cuda-a100", producer_role="matmul", consumer_role="silu",
        decision="fuse", predicted_speedup=1.5, observed_speedup=2.0,
    ))
    db.record_fusion_decision(FusionDecisionRecord(
        target="cuda-h100", producer_role="matmul", consumer_role="silu",
        decision="fuse", predicted_speedup=1.5, observed_speedup=1.2,
    ))
