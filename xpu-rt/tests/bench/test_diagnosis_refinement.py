...
def _matmul_contract() -> KernelContractV3:
    ...
    lhs = TensorIO(name="lhs", shape=ShapeClass(dims=(None, None)), dtype_class=("f16",))
    rhs = TensorIO(name="rhs", shape=ShapeClass(dims=(None, None)), dtype_class=("f16",))
    out = TensorIO(name="out", shape=ShapeClass(dims=(None, None)), dtype_class=("f16",))
    return KernelContractV3(
        op_name="linalg.matmul",
        archetype=KernelArchetype.COMPUTE_TILED,
        io=IOContract(inputs=(lhs, rhs), outputs=(out,), numerics=NumericsSpec(accumulator_dtype="f32")),
...
def _softmax_contract() -> KernelContractV3:
    ...
    inp = TensorIO(name="x", shape=ShapeClass(dims=(None, None)), dtype_class=("f32",))
    out = TensorIO(name="y", shape=ShapeClass(dims=(None, None)), dtype_class=("f32",))
    return KernelContractV3(
        op_name="softmax",
...
def test_correctness_failure_dominates_diagnosis() -> None:
    ...
    d = diagnose(_matmul_contract(), _bench("m1", our_us=100, eager_us=40, passed=False, max_abs_err=0.5))
...
def test_compute_tiled_slow_kernel_suggests_autotune() -> None:
    ...
    d = diagnose(_matmul_contract(), _bench("m2", our_us=194, eager_us=37))
...
def test_reduce_archetype_gets_reduce_specific_hypotheses() -> None:
    ...
    d = diagnose(_softmax_contract(), _bench("s1", our_us=120, eager_us=40, input_shapes=[[2048, 1024]]))
    joined = " ".join(d.hypotheses).lower()
