...
1. ``u6 should be of base attribute builtin.int`` — ``torch.export``
   ...
   rejects them. :func:`xpu_rt.ir.payload.decompositions._static_shape`
...
2. ``NotImplementedError`` from ``BFloat16Type.pack`` — SmolVLA's
   ...
   opaque ``_xpu_rt_cast_scalar`` call carrying the original dtype.
"""

from __future__ import annotations

from xpu_rt.ir.payload.decompositions import _scalar_to_tensor, _static_shape
...
def test_scalar_to_tensor_bf16_falls_back_to_cast() -> None:
    ...
    assert ssa.type == like
    assert "xpu_rt.cast_to" in cast.attributes
