...
from xpu_rt.ir.payload.decompositions import DecompResult
from xpu_rt.ir.payload.import_fx import FXImporter, _forward_fx_meta
...
def test_forward_fx_meta_pattern_hint_only():
    op = _make_op()
    _forward_fx_meta(op, {"_xpu_rt_pattern": "softmax"})
    assert op.attributes["xpu_rt._pattern_hint"].data == "softmax"


def test_forward_fx_meta_decomp_hint_wins_when_fx_absent():
    op = _make_op()
    _forward_fx_meta(op, {}, decomp_hint="rms_norm")
    assert op.attributes["xpu_rt._pattern_hint"].data == "rms_norm"


def test_forward_fx_meta_fx_hint_wins_over_decomp():
    op = _make_op()
    _forward_fx_meta(op, {"_xpu_rt_pattern": "layer_norm"}, decomp_hint="other")
    assert op.attributes["xpu_rt._pattern_hint"].data == "layer_norm"


def test_forward_fx_meta_is_idempotent():
    op = _make_op()
    op.attributes["xpu_rt._pattern_hint"] = StringAttr("pre_existing")
    _forward_fx_meta(op, {"_xpu_rt_pattern": "other"})
    # Idempotent: never overwrites
    assert op.attributes["xpu_rt._pattern_hint"].data == "pre_existing"
...
def test_forward_fx_meta_transpose_absorbed_and_fuse_dequant():
    ...
    _forward_fx_meta(
        op,
        {
            "_xpu_rt_pattern": "matmul",
            "_xpu_rt_transpose_absorbed": True,
            "_xpu_rt_fuse_dequant": True,
        },
    )
    assert op.attributes["xpu_rt.transpose_absorbed"].data == "true"
    assert op.attributes["xpu_rt.fuse_dequant"].data == "true"
