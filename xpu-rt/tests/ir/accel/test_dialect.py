...
from xpu_rt.ir.accel.dialect import AccelDialect, AccelDialectConfig
...
def test_accel_dialect_name() -> None:
    ...
    assert AccelDialect.name == "xpu_rt.accel"
...
def test_accel_dialect_config_defaults() -> None:
    ...
    assert config.name == "xpu_rt.accel"
...
def test_accel_dialect_ops_registered() -> None:
    ...
    for op_cls in ACCEL_IR_OPS:
        assert op_cls.name.startswith("xpu_rt.accel.")
