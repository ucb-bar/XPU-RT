"""Tests for the ``xpu_rt tool`` CLI subcommand.

Coverage:

Positive:
* ``xpu_rt tool list`` returns the registered cards as JSON.
* ``xpu_rt tool describe <id>`` returns the full card body.
* ``xpu_rt tool run <id> --input file --out dir`` runs the tool,
  emits the ToolResult on stdout, and exits 0 on ``status=ok``.
* ``xpu_rt tool run <id> --stdin`` accepts the request on stdin.
...
def _run_cli(*args: str, input_text: str | None = None, env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    """Invoke ``xpu-rt-tool`` via the venv-installed entrypoint.

    Falls back to ``python -m xpu_rt.cli tool ...`` if the entrypoint
    isn't on PATH yet (e.g., editable-install cache lag in CI).
    """

    cmd = [shutil.which("xpu-rt-tool") or "xpu-rt-tool", *args]
    if cmd[0] is None or not Path(cmd[0]).exists():
        cmd = [sys.executable, "-m", "xpu_rt.cli", "tool", *args]
...
def test_list_phase_filter():
    ...
    assert any(row["tool_id"] == "xpu_rt_echo" for row in body["tools"])


def test_list_phase_filter_excludes_others():
    proc = _run_cli("list", "--phase", "kernel_codegen")
    body = _json_or_fail(proc)
    assert all(row["tool_id"] != "xpu_rt_echo" for row in body["tools"])
...
def test_describe_returns_full_card():
    proc = _run_cli("describe", "xpu_rt_echo")
    assert proc.returncode == 0, proc.stderr
    body = _json_or_fail(proc)
    assert body["status"] == "ok"
    card = body["tool"]
...
def test_run_positive(tmp_path):
    ...
    out = tmp_path / "out"
    proc = _run_cli("run", "xpu_rt_echo", "--input", str(req), "--out", str(out))
    assert proc.returncode == 0, proc.stderr
    body = _json_or_fail(proc)
    assert body["status"] == "ok"
    assert body["tool_id"] == "xpu_rt_echo"
...
def test_run_byte_stable_output_hash(tmp_path):
    ...
    out = tmp_path / "out"
    a = _json_or_fail(_run_cli("run", "xpu_rt_echo", "--input", str(req), "--out", str(out)))
    b = _json_or_fail(_run_cli("run", "xpu_rt_echo", "--input", str(req), "--out", str(out)))
...
def test_run_missing_request_source(tmp_path):
    proc = _run_cli("run", "xpu_rt_echo", "--out", str(tmp_path / "out"))
...
def test_run_input_file_with_bad_json(tmp_path):
    ...
    proc = _run_cli("run", "xpu_rt_echo", "--input", str(req), "--out", str(tmp_path / "out"))
    assert proc.returncode == 2
    body = _json_or_fail(proc)
...
def test_run_unknown_tool_id(tmp_path):
    ...
    assert "xpu_rt_echo" in body["available"]
...
def test_run_output_schema_violation_via_isolated_cards(tmp_path):
    ...
    body["tool_id"] = "xpu_rt_echo_bad_status_cli"
    body["entrypoints"]["python"] = "xpu_rt.tools.builtin.echo:returns_bad_status"
    body["entrypoints"]["cli"] = "xpu-rt-tool run xpu_rt_echo_bad_status_cli"
    ...
    proc = _run_cli(
        ...
        "run", "xpu_rt_echo_bad_status_cli",
        "--input", str(req),
        "--out", str(tmp_path / "out"),
    )
...
def test_run_entrypoint_import_error(tmp_path):
    ...
    cards_dir.mkdir()
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_echo_missing_module_cli"
    body["entrypoints"]["python"] = "xpu_rt.totally_fake_module:run"
    body["entrypoints"]["cli"] = "xpu-rt-tool run xpu_rt_echo_missing_module_cli"
    ...
    proc = _run_cli(
        "--cards-root", str(cards_dir),
        "run", "xpu_rt_echo_missing_module_cli",
        "--input", str(req),
        "--out", str(tmp_path / "out"),
    )
...
def test_run_entrypoint_crash(tmp_path):
    ...
    cards_dir.mkdir()
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_echo_crash_cli"
    body["entrypoints"]["python"] = "xpu_rt.tools.builtin.echo:crashes"
    body["entrypoints"]["cli"] = "xpu-rt-tool run xpu_rt_echo_crash_cli"
...
def test_run_status_error_exits_one(tmp_path):
    ...
    cards_dir.mkdir()
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_echo_status_error_cli"
    body["entrypoints"]["cli"] = "xpu-rt-tool run xpu_rt_echo_status_error_cli"
    ...
    proc = _run_cli(
        "--cards-root", str(cards_dir),
        "run", "xpu_rt_echo_status_error_cli",
        "--input", str(req),
        "--out", str(tmp_path / "out"),
    )
    assert proc.returncode == 0, proc.stderr
...
def test_audit_promotion_runs_clean_on_shipped_cards():
    """``xpu_rt tool audit-promotion`` dispatches to and reports
    ...
    echo_outcomes = [o for o in body["outcomes"] if o["tool_id"] == "xpu_rt_echo"]
