"""Tests for :mod:`xpu_rt.mcp.tool_bridge`.
...
* The bridged tool appears inside ``xpu_rt.mcp.tools.get_all_tools()``
...
import pytest
import yaml
from xpu_rt.mcp.tool_bridge import (
    bridge_tools,
    make_mcp_tool_dict,
)
...
def test_echo_bridge_tool_round_trips():
    ...
    assert tool["name"] == "xpu_rt_echo"
    assert tool["phase"] in {"inspect", "transform", "lifecycle", "job"}
    assert callable(tool["handler"])
    assert tool["_card_tool_id"] == "xpu_rt_echo"
...
def test_bridged_tool_reachable_via_get_all_tools():
    from xpu_rt.mcp.tools import get_all_tools

    names = [t.get("name") for t in get_all_tools()]
    assert "xpu_rt_echo" in names
...
def test_bridge_handler_runs_end_to_end(tmp_path):
    ...
    assert payload["tool_id"] == "xpu_rt_echo"
...
def test_make_mcp_tool_dict_rejects_card_without_mcp_entrypoint():
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_no_mcp_direct"
    body["entrypoints"]["mcp"] = ""
...
def test_bridge_handler_translates_run_error_to_typed_payload(tmp_path):
    ...
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_bridge_crashes"
    body["entrypoints"]["python"] = "xpu_rt.tools.builtin.echo:crashes"
    body["entrypoints"]["mcp"] = "xpu_rt_bridge_crashes"
    card = ToolCard.from_dict(body)
    handler = make_mcp_tool_dict(card)["handler"]
...
def test_bridge_handler_translates_entrypoint_import_error(tmp_path):
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_bridge_missing"
    body["entrypoints"]["python"] = "xpu_rt.totally_fake_xyz:run"
    body["entrypoints"]["mcp"] = "xpu_rt_bridge_missing"
    card = ToolCard.from_dict(body)
    handler = make_mcp_tool_dict(card)["handler"]
    payload = handler(sm=None, text="x", out_dir=str(tmp_path / "out"))
    assert payload["status"] == "error"
...
def test_bridge_handler_translates_output_schema_violation(tmp_path):
    body = yaml.safe_load((tool_cards_root() / "echo.yaml").read_text(encoding="utf-8"))
    body["tool_id"] = "xpu_rt_bridge_bad_output"
    body["entrypoints"]["python"] = "xpu_rt.tools.builtin.echo:returns_bad_status"
    body["entrypoints"]["mcp"] = "xpu_rt_bridge_bad_output"
    card = ToolCard.from_dict(body)
    handler = make_mcp_tool_dict(card)["handler"]
    payload = handler(sm=None, text="x", out_dir=str(tmp_path / "out"))
    assert payload["status"] == "error"
...
def test_bridge_module_references_tool_runner():
    ...
    from xpu_rt.mcp import tool_bridge
