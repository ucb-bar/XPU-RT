...
def _copy_op(op: Operation) -> Operation:
    new = Operation(
        ...
        # Predecessor-aware cost tensor, keyed by (pred_combo_idx,
        ...
        # is compared against.
        processing_times_by_pred=dict(
            getattr(op, "processing_times_by_pred", None) or {}),
    )
...
def _apply_fuse_pair(workload, prod_idx, cons_idx):
    ...
    # The inherited per-predecessor cost map describes the PRODUCER's cost,
    # which `processing_times` above has just replaced with the pair's sum.
    # Keeping it would feed the scheduler a tensor that contradicts the 2D
    # costs; clearing it takes the documented empty-map fallback instead.
    fused.processing_times_by_pred = {}
...
def _apply_split(workload, op_idx, n_splits):
    ...
    for s in range(n_splits):
        ...
        # Same reason as the fused op: the copied map describes the WHOLE
        # op, not this 1/n tile, so it must not survive the cost rewrite.
        part.processing_times_by_pred = {}
