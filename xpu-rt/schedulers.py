...
# The learned/RL/GNN/LLM-ranker schedulers (scheduler_ml, scheduler_gnn,
...
def compaction_enabled() -> bool:
    """Whether get_scheduler() will apply the left-shift compaction post-pass.

    Compaction is OPT-IN (env XPURT_COMPACT=1). It was originally on by
    default, but a post-pass that silently changes a schedule's makespan
    makes every cross-policy comparison actually a comparison of
    policy+compaction: list schedulers gain a lot from it and tight solvers
    gain nothing, so leaving it on by default biases the ranking. Callers
    that want it must say so, and should record this flag in their manifest.

    XPURT_NO_COMPACT=1 is still honoured as an explicit force-off.
    """
    if os.environ.get("XPURT_NO_COMPACT", "0") in ("1", "true", "True"):
        return False
    return os.environ.get("XPURT_COMPACT", "0") in ("1", "true", "True")
...
def _wrap_with_compaction(fn: SchedulerFn) -> SchedulerFn:
    ...
    Off unless XPURT_COMPACT=1; see compaction_enabled()."""
