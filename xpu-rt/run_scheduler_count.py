"""
CLI for the count-only MILP scheduler (scheduler_count.schedule_by_count +
greedy specific-core assignment).
"""
...
from scheduler_count import schedule_with_count_and_assignment
...
def run(
    ...
    machines, combos = build_machine_combinations(machine_core_counts)
    ...
    for combo in combos:
        ...
        combo_hw.append(cfg["cpu_p_profile_hw"] if t == CPU_P else cfg["cpu_e_profile_hw"])
    ...
    print(f"Workload: {len(workload.operations)} ops, {len(combos)} cumulative combos")

    expanded_workload, t, alpha = schedule_with_count_and_assignment(
        workload=workload,
    ...
    exp_combos = expanded_workload.get_machine_combinations()
    completion = []
    for i, op in enumerate(expanded_workload.operations):
        k = int(np.argmax(alpha[i]))
        d = op.get_duration_for_combination(k, exp_combos, expanded_workload.machines)
        if op.min_start_t is None and op.max_end_t is None:
            completion.append(float(t[i]) + float(d))
    ...
    title = " + ".join(n.capitalize() for n in names)
    plot.plot_optimization_schedule(
        ...
        save_path="plots/scheduler_count_schedule.png",
        plot_title=f"{title} Count-MILP Schedule ({n_cores} cores)",
        workload=expanded_workload,
    )
    print("Plot -> plots/scheduler_count_schedule.png")

    os.makedirs("schedules", exist_ok=True)
    base = os.path.splitext(os.path.basename(networks_json_path))[0]
    out = f"schedules/scheduler_count_{base}.json"
    if cfg["use_profiled"]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Count-only MILP scheduler")
