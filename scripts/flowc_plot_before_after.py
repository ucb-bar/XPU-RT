...
def main() -> int:
    ...
    fig2, ax2 = plt.subplots(figsize=(9.2, 4.2))
    ...
    for i, k in enumerate(knobs):
        ...
        for j, net in enumerate(order):
            rs = [r for r in nets[net] if r["ms"] is not None]
            idx = next((q for q, r in enumerate(rs) if short(r["knob"]) == k), None)
            saved = (rs[idx - 1]["ms"] - rs[idx]["ms"]) if (idx and idx > 0) else 0.0
            vals.append(saved)
            xs.append(j + i * width)
        ax2.bar(xs, vals, width * 0.9, color=KNOB_COLOR[k], label=k,
                edgecolor="#fcfcfb", lw=1.2, zorder=3)
        for x, v in zip(xs, vals):
            if abs(v) > 1e-9:
                ax2.annotate(f"{v:+.3g}", (x, v), xytext=(0, 4 if v >= 0 else -12),
                             textcoords="offset points", ha="center",
                             fontsize=7.6, color=INK)
    ax2.axhline(0, color=INK, lw=0.9)
    ax2.set_xticks([j + 1.5 * width for j in range(len(order))])
    ax2.set_xticklabels(order, fontsize=10, color=INK)
    ax2.set_ylabel("milliseconds removed by the knob", fontsize=9, color=MUTED)
    ax2.set_yscale("symlog", linthresh=0.05)
    # widen the negative side so the "knob cost time" labels clear the x ticks
    lo, hi = ax2.get_ylim()
    ax2.set_ylim(lo * 2.6, hi * 1.5)
    ...
    ax2.legend(fontsize=8.5, frameon=False, ncol=4, loc="upper right")
    ax2.set_title("Which knob removed the time — bars below zero are knobs that COST time\n"
                  "symlog axis: the five networks differ by three orders of magnitude",
                  fontsize=11.5, loc="left", color=INK, pad=14)
    fig2.tight_layout()
