"""Tests for ``xpu_rt.options.XpuRtOptions`` + presets."""
...
def test_default_numerics_policy_preserves_f32():
    assert XpuRtOptions().numerics_policy == "preserve_f32"


def test_default_scheduling_policy_is_static():
    assert XpuRtOptions().scheduling_policy == "static"
...
def test_to_dict_round_trip_preserves_equality():
    ...
    round = XpuRtOptions.from_dict(orig.to_dict())
    assert orig == round


def test_to_dict_converts_frozenset_to_sorted_list():
    opts = XpuRtOptions(kernel_family_allowlist=frozenset({"triton", "cublas"}))
    out = opts.to_dict()
...
def test_replace_preserves_frozenset_fields():
    a = XpuRtOptions(library_allowlist=frozenset({"triton", "cublas"}))
