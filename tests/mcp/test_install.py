"""Tests for :mod:`xpu_rt.mcp.install` — config merge with backup.

Locks in:
  * creates a fresh config when the target file does not exist
  * preserves existing ``mcpServers`` entries while adding the xpu_rt one
...
import pytest
from xpu_rt.mcp.config import mcp_server_entry
...
def test_install_creates_fresh_file(tmp_path: Path):
    ...
    assert data == {"mcpServers": {"xpu-rt": mcp_server_entry()}}
