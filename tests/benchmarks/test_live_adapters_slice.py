...
1. ``LiveXpuRtAdapter`` returns a typed ``blocked`` measurement on
...
2. ``LiveTorchEagerAdapter`` produces measurable latencies on the
   TinyLlama MLP slice.
3. ``LiveXpuRtAdapter`` produces measurable latencies on the
...
def test_xpu_rt_blocks_on_full_tinyllama() -> None:
    from xpu_rt.benchmarks.live_adapters import LiveXpuRtAdapter

    m = LiveXpuRtAdapter().measure(
