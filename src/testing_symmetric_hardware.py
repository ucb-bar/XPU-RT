...
import plot
from scheduler import schedule, schedule_with_convex_packing, schedule_with_greedy_packing
...
def transfer_time_test_new(type="convex"):
    machines = {"cpu" : 3, "gpu" : 3, "tpu" : 2}
    transfer_times = np.array([
        [0, 10, 50],
        [10, 0, 200],
        [50, 200, 0]
    ...
    if type == "convex":
        t, alpha = schedule_with_convex_packing(workload, n_splits=3)
    else:
...
if __name__ == "__main__":
    #transfer_time_test()
    transfer_time_test_new(type="convex")
