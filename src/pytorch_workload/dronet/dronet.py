...
class DronetTorch(nn.Module):
    def __init__(self, img_dims, img_channels, output_dim):
        ...
        self.conv_modules.append(
            nn.Conv2d(self.channels, 32, (5, 5), stride=(2, 2), padding=(2, 2))
        )
        ...
        for f in filter_amt:
            ...
            x2 = f
            self.conv_modules.append(
                nn.Conv2d(x1, x2, (3, 3), stride=(2, 2), padding=(1, 1))
            )
        ...
        # Dummy forward to determine linear input size
        with torch.no_grad():
            dummy = torch.zeros(1, self.channels, *self.img_dims)
            x = self.conv_modules[0](dummy)
            x1 = self.maxpool1(x)
            bn_idx = 0
            conv_idx = 1
            relu_idx = 0
            for i in range(3):
                ...
                relu_idx += 2
                conv_idx += 3
            num_features = x3.numel() // x3.shape[0]
        self.linear1 = nn.Linear(num_features, output_dim)
...
def extract_layer_info(model: nn.Module, input_shape):
    ...
    def add_hook(a, b):
        ...
        rec = {
            ...
            "input_from": out_map.get(id(a), "input")
            + ","
            + out_map.get(id(b), "input"),
        }
        info.append(rec)
        out_map[id(out)] = name
        return out
    ...
    for name, module in model.named_modules():
        if isinstance(
            module, (nn.Conv2d, nn.MaxPool2d, nn.Linear, nn.ReLU, nn.Sigmoid)
        ):
...
# For chaining, set img_channels=1 for grayscale input
model = DronetTorch(img_dims=(224, 224), img_channels=1, output_dim=1)
layer_info = extract_layer_info(model, input_shape=(1, 1, 224, 224))

# Example ONNX export for 224x224 input
input_tensor = torch.randn(1, 1, 224, 224)  # Example input tensor
torch.onnx.export(
    model,  # model to export
    (input_tensor,),  # inputs of the model,
    "dronet_fixed.onnx",  # filename of the ONNX model
