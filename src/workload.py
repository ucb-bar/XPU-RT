...
class Workload:
    ...
    def __init__(
        ...
        if not all_machines_in_combinations.issubset(set(machines)):
            raise ValueError(
                f"Machine combinations contain machines not in the machines list: {all_machines_in_combinations - set(machines)}"
            )
    ...
    def get_durations(self) -> list:
        ...
        for i in range(len(self.operations)):
            ...
            if operation.job_id is not None:
                ...
                if (
                    not operation.predecessors
                ):  # No predecessors means start of a new job
                    durations.append([operation.get_durations()])
