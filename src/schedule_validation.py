    ...
    for i in range(len(t)):
        for j in range(i + 1, len(t)):
            ...
            if t[i] < t[j] and np.argmax(alpha[i]) == np.argmax(alpha[j]):
                if (
                    t[i]
                    + workload.operations[i].get_durations()[np.argmax(alpha[i])]
                    + transfer_time
                    > t[j]
                ):
                    t[j] = (
                ...
                if (
                    t[j]
                    + workload.operations[j].get_durations()[np.argmax(alpha[j])]
                    + transfer_time
                    > t[i]
                ):
                    t[i] = (
...
def validate_schedule(
    ...
    if missing_ops:
        ...
        results["errors"].append(
            f"Missing {len(missing_ops)} operations from JSON: {sorted(list(missing_ops))[:10]}"
        )

    if extra_ops:
        results["warnings"].append(
            f"Extra {len(extra_ops)} operations in schedule not in JSON: {sorted(list(extra_ops))[:10]}"
        )
    ...
    for i, op in enumerate(workload.operations):
        ...
        for json_dep in json_deps:
            if json_dep not in actual_pred_names:
                ...
                if json_dep in op_name_to_idx:
                    ...
                    pred_finish_time = t[pred_idx] + workload.operations[
                        pred_idx
                    ].get_duration_for_combination(
                        np.argmax(alpha[pred_idx]), machine_combinations, machines
                    )
                    ...
                    results["warnings"].append(
                        f"Operation {op_name} depends on {json_dep} (from JSON) but it's not in the schedule"
                    )
        ...
        for pred in actual_predecessors:
            try:
                ...
                if t[i] < pred_finish_time:
                    pred_name = (
                        pred.operation_name or pred.operation_id or f"op{pred_idx}"
                    )
                    predecessor_violations.append(
                        {
                            "operation": op_name,
    ...
    if predecessor_violations:
        ...
        results["errors"].append(
            f"Found {len(predecessor_violations)} predecessor constraint violations"
        )
    ...
    for i, op in enumerate(workload.operations):
        ...
        scheduled_duration = op.get_duration_for_combination(
            assigned_combo_idx, machine_combinations, machines
        )

        # Also get the processing_times to see what's stored
        processing_times = (
            op.processing_times if hasattr(op, "processing_times") else []
        )
        ...
        if (
            "CPU_P" in assigned_combo
            and network_profiled_p
            and dispatch_id in network_profiled_p
        ):
            ...
            if (
                cpu_p_stored is not None
                and abs(scheduled_duration - cpu_p_stored) < 0.01
            ):
                if (
                    abs(scheduled_duration - profiled_duration) > 0.01
                ):  # Allow 0.01ms tolerance
        ...
        if (
            "CPU_E" in assigned_combo
            and network_profiled_e
            and dispatch_id in network_profiled_e
        ):
            ...
            # Only flag mismatch if scheduled matches stored (ensures operation is correct)
            if (
                cpu_e_stored is not None
                and abs(scheduled_duration - cpu_e_stored) < 0.01
            ):
                if (
                    abs(scheduled_duration - profiled_duration) > 0.01
                ):  # Allow 0.01ms tolerance
    ...
    if duration_mismatches:
        ...
        results["warnings"].append(
            f"Found {len(duration_mismatches)} duration mismatches with profiled data"
        )
    ...
    if invalid_assignments:
        ...
        results["errors"].append(
            f"Found {len(invalid_assignments)} invalid machine assignments"
        )
        ...
        for j in range(i + 1, num_operations):
            ...
            if set(combo_i) & set(combo_j):  # Overlapping machines
                ...
                dur_i = workload.operations[i].get_duration_for_combination(
                    np.argmax(alpha[i]), machine_combinations, machines
                )
                dur_j = workload.operations[j].get_duration_for_combination(
                    np.argmax(alpha[j]), machine_combinations, machines
                )
                ...
                if not (finish_i <= t[j] or finish_j <= t[i]):
                    overlaps.append(
                        {
                            "operation1": workload.operations[i].operation_name
                            or workload.operations[i].operation_id
                            or f"op{i}",
                            "operation2": workload.operations[j].operation_name
                            or workload.operations[j].operation_id
                            or f"op{j}",
        ...
        results["operation_details"].append(
            {
                ...
                "machine": (
                    assigned_combo[0]
                    if len(assigned_combo) == 1
                    else "+".join(assigned_combo)
                ),
            }
        )
...
def write_validation_report(
    ...
    dispatches: Dict = None,
    profiled_times_p: Dict = None,
    profiled_times_e: Dict = None,
    machine_combinations: List = None,
    machines: List = None,
):
    ...
    with open(output_file, "w") as f:
        ...
        f.write("-" * 80 + "\n")
        f.write(
            f"All operations scheduled: {'PASS' if results['all_operations_scheduled'] else 'FAIL'}\n"
        )
        f.write(
            f"Predecessor constraints satisfied: {'PASS' if results['predecessor_constraints_satisfied'] else 'FAIL'}\n"
        )
        f.write(
            f"Durations match profiled data: {'PASS' if results['durations_match_profiled'] else 'FAIL'}\n"
        )
        f.write(
            f"Machine assignments valid: {'PASS' if results['machine_assignments_valid'] else 'FAIL'}\n"
        )
        ...
        if (
            "duration_mismatch_details" in results
            and results["duration_mismatch_details"]
        ):
            f.write("DURATION MISMATCHES (Detailed)\n")
            f.write("-" * 80 + "\n")
            f.write(
                "NOTE: Mismatches may occur when the same dispatch_id appears in multiple networks.\n"
            )
            f.write(
                "The profiled data may be from a different network than the operation.\n"
            )
            f.write("-" * 80 + "\n")
            f.write(
                f"{'Operation':<35} {'Dispatch ID':<12} {'Network':<12} {'Machine':<8} {'Scheduled':<12} {'Profiled':<12} {'Diff':<12} {'Stored P/E':<12}\n"
            )
            f.write("-" * 80 + "\n")
            mismatches = results["duration_mismatch_details"]
            for mm in sorted(mismatches, key=lambda x: x["difference"], reverse=True)[
                :30
            ]:  # Top 30
                stored_val = mm.get("processing_times_cpu_p") or mm.get(
                    "processing_times_cpu_e"
                )
        ...
        if "overlap_details" in results and results["overlap_details"]:
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"{'Operation 1':<35} {'Operation 2':<35} {'Machine':<10} {'Op1 Time':<20} {'Op2 Time':<20}\n"
            )
            f.write("-" * 80 + "\n")
            for overlap in results["overlap_details"]:
                op1_time_str = (
                    f"[{overlap['op1_time'][0]:.3f}, {overlap['op1_time'][1]:.3f}]"
                )
                op2_time_str = (
                    f"[{overlap['op2_time'][0]:.3f}, {overlap['op2_time'][1]:.3f}]"
                )
                f.write(
        ...
        f.write("-" * 80 + "\n")
        f.write(
            f"{'Index':<8} {'Name':<30} {'Dispatch ID':<12} {'Start':<10} {'Duration':<10} {'Finish':<10} {'Machine':<10}\n"
        )
        f.write("-" * 80 + "\n")
        for op_detail in results["operation_details"]:
            dispatch_id_str = (
                str(op_detail["dispatch_id"])
                if op_detail["dispatch_id"] is not None
                else "N/A"
            )
            f.write(
                f"{op_detail['index']:<8} "
