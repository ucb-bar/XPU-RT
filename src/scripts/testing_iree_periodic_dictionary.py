...
def output_scheduled_json(
    ...
    for op_idx in range(len(combined_workload.operations)):
        ...
        job_name = (
            combined_workload.job_names[job_id]
            if job_id < len(combined_workload.job_names)
            else f"Job {job_id}"
        )
    ...
    for info in dispatch_info_list:
        ...
        for pred_op in op.predecessors:
            ...
            if pred_idx is not None:
                pred_dispatch_name = (
                    combined_workload.operations[pred_idx].operation_name
                    if hasattr(combined_workload.operations[pred_idx], "operation_name")
                    and combined_workload.operations[pred_idx].operation_name
                    else f"op_{pred_idx}"
                )
...
def _trim_periodic_after_nonperiodic_makespan(
    workload: Workload, t: np.ndarray, alpha: np.ndarray
    ...
    for i, op in enumerate(workload.operations):
        is_periodic = (getattr(op, "min_start_t", None) is not None) or (
            getattr(op, "max_end_t", None) is not None
        )
        if is_periodic:
            continue
        ...
        dur = op.get_duration_for_combination(
            combo_idx, workload.get_machine_combinations(), workload.machines
        )
...
def schedule_iree_networks(
    ...
    if networks_json_path is None:
        networks_json_path = os.path.join(
            script_dir, "..", "data", "toplevel", "networks_periodic.json"
        )
    ...
    if hardware_json_path is None:
        hardware_json_path = os.path.join(
            script_dir, "..", "data", "hardware", "hardware_spec.json"
        )
    ...
    for network_id, network_info in networks.items():
        print(
            f"  - {network_id} (id: {network_info.get('id')}, identifier: {network_info.get('identifier')})"
        )
    ...
    if use_profiled:
        ...
        def get_profiles_for_dispatch_path(
            path: str,
    ...
    if prune_periodic:
        combined_workload, t, alpha = _trim_periodic_after_nonperiodic_makespan(
    ...
    for job_id in sorted(network_stats.keys()):
        ...
        print(
            f"  {stats['name'].capitalize()}: CPU_P={stats['cpu_p']}, CPU_E={stats['cpu_e']}"
        )
    ...
    network_names = [
        (
            combined_workload.job_names[i]
            if i < len(combined_workload.job_names)
            else f"Job {i}"
        )
