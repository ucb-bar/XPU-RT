...
class OperationWithCombinations(Operation):
    ...
    def __init__(
        ...
        """
        super().__init__(
            processing_times, predecessors, operation_id, operation_name, job_id
        )
        self.combined_duration = combined_duration

    def get_duration_for_combination(
        self,
        combination_idx: int,
...
def greedy_schedule(workload: Workload) -> tuple:
    ...
    )  # Combination assignments (one-hot)
    ...
    while not all(scheduled):
            ...
            for combo_idx in range(num_combinations):
                ...
                for j in range(num_operations):
                    ...
                    if workload.combinations_overlap(combo_idx, scheduled_combo_idx):
                        ...
                        scheduled_op_duration = workload.operations[
                            j
                ...
                for pred in op.predecessors:
                    ...
                    pred_duration = workload.operations[
                        pred_idx
                    ].get_duration_for_combination(
                    ...
                    transfer_time = transfer_times[
                        pred_machine_idx, candidate_machine_idx
                    ]
        ...
        for combo_idx in range(num_combinations):
            if combo_idx != best_combination_idx and workload.combinations_overlap(
                best_combination_idx, combo_idx
            ):
                combination_available_time[combo_idx] = max(
                    combination_available_time[combo_idx], operation_end_time
                )
...
def create_workload_from_json_with_combinations(
    ...
    workload = Workload(
        operations, machines, transfer_times, machine_combinations=machine_combinations
    )
...
def combine_workloads(
    workloads: list, job_names: list = None, job_id_mapping: list = None
) -> Workload:
    """
...
def add_dependency(source_workload: Workload, target_workload: Workload):
    ...
    if not target_first_ops:
        target_first_ops = (
            [target_workload.operations[0]] if target_workload.operations else []
        )
...
def schedule_iree_networks(
    ...
    base_path = os.path.join(script_dir, "..", "pytorch_workload", "samples")
    ...
    if use_profiled:
        ...
        if not no_depth_network:
            if use_glpdepth:
                ...
                first_network_profiled_times_p = load_profiled_times(
                    glpdepth_profile_csv_p
                )
                print(
                    f"   Loaded {len(first_network_profiled_times_p)} profiled P-core entries"
                )

                print(
                    f"\nLoading profiled glpdepth E-core runtimes from: {glpdepth_profile_csv_e}"
                )
                first_network_profiled_times_e = load_profiled_times(
                    glpdepth_profile_csv_e
                )
                print(
                    f"   Loaded {len(first_network_profiled_times_e)} profiled E-core entries"
                )
        ...
        if use_mobilenet:
            ...
            print(
                f"   Loaded {len(mobilenet_profiled_times_p)} profiled P-core entries"
            )
            ...
            print(
                f"   Loaded {len(mobilenet_profiled_times_e)} profiled E-core entries"
            )
        ...
        if use_diffusion:
            ...
            print(
                f"   Loaded {len(diffusion_profiled_times_p)} profiled P-core entries"
            )
            ...
            print(
                f"   Loaded {len(diffusion_profiled_times_e)} profiled E-core entries"
            )
    ...
    if mlp_debug:
        ...
        if not no_depth_network:
            ...
            print(
                f"   Created {first_network_job_name} workload with {len(first_network_workload.operations)} operations"
            )
        else:
    ...
    if not use_mobilenet and not use_diffusion:
        ...
        for i in range(num_mlps):
            ...
            mlp_workload, mlp_job_name = create_workload_func(
                mlp_path, name_prefix=mlp_prefix
            )
    ...
    if not use_mobilenet and not use_diffusion:
        ...
        if len(mlp_workloads) > 1:
            first_network_label = (
                first_network_name.capitalize() if not no_depth_network else "Dronet"
            )
            print(
    ...
    if mlp_debug:
        ...
        print(
            f"     Chain 1: {' → '.join(mlp_job_names) if len(mlp_job_names) > 1 else mlp_job_names[0] + ' (independent)'}"
        )
        ...
        if not use_mobilenet and not use_diffusion:
            if not no_depth_network:
                all_workloads = [
                    first_network_workload,
                    dronet_workload,
                ] + mlp_workloads
                all_job_names = [
                    first_network_job_name,
                    dronet_job_name,
                ] + mlp_job_names
                # Create job_id mapping: First network=0, Dronet=1, all MLPs=2 (same color for all MLPs)
                job_id_mapping = [0, 1] + [2] * len(
                    mlp_workloads
                )  # All MLPs get job_id=2
                # Use a single name "MLP" for all MLP instances in the legend
                all_job_names_for_legend = [first_network_job_name, dronet_job_name] + [
                    "MLP"
                ] * len(mlp_workloads)
            else:
                ...
                # Use a single name "MLP" for all MLP instances in the legend
                all_job_names_for_legend = [dronet_job_name] + ["MLP"] * len(
                    mlp_workloads
                )
        else:
    ...
    combined_workload = combine_workloads(
        all_workloads, job_names=all_job_names_for_legend, job_id_mapping=job_id_mapping
    )
    ...
    if not mlp_debug:
        if not no_depth_network:
            first_network_indices = list(
                range(current_idx, current_idx + len(first_network_workload.operations))
            )
            current_idx += len(first_network_workload.operations)

        # Mobilenet and diffusion come before dronet when enabled
        if use_mobilenet:
            mobilenet_indices = list(
                range(current_idx, current_idx + len(mobilenet_workload.operations))
            )
            current_idx += len(mobilenet_workload.operations)
        if use_diffusion:
            diffusion_indices = list(
                range(current_idx, current_idx + len(diffusion_workload.operations))
            )
            current_idx += len(diffusion_workload.operations)

        if dronet_workload:
            dronet_indices = list(
                range(current_idx, current_idx + len(dronet_workload.operations))
            )
    ...
    if not use_mobilenet and not use_diffusion:
        for mlp_workload in mlp_workloads:
            mlp_indices = list(
                range(current_idx, current_idx + len(mlp_workload.operations))
            )
    ...
    for combo_idx in range(num_combinations):
        ...
        if mlp_debug:
            ...
            for i, mlp_indices in enumerate(mlp_indices_list):
                mlp_count = sum(
                    1 for idx in mlp_indices if np.argmax(alpha[idx]) == combo_idx
                )
                print(f"    {mlp_job_names[i].capitalize()}: {mlp_count} operations")
        else:
            if not no_depth_network and first_network_indices:
                first_network_count = sum(
                    1 for i in first_network_indices if np.argmax(alpha[i]) == combo_idx
                )
                print(
                    f"    {first_network_job_name.capitalize()}: {first_network_count} operations"
                )
            if dronet_indices:
                dronet_count = sum(
                    1 for i in dronet_indices if np.argmax(alpha[i]) == combo_idx
                )
            ...
            if not use_mobilenet and not use_diffusion:
                for i, mlp_indices in enumerate(mlp_indices_list):
                    ...
                    print(
                        f"    {mlp_job_names[i].capitalize()}: {mlp_count} operations"
                    )
            else:
                if use_mobilenet:
                    mobilenet_count = sum(
                        1
                        for idx in mobilenet_indices
                        if np.argmax(alpha[idx]) == combo_idx
                    )
                    print(
                        f"    {mobilenet_job_name.capitalize()}: {mobilenet_count} operations"
                    )
                if use_diffusion:
                    diffusion_count = sum(
                        1
                    ...
                    print(
                        f"    {diffusion_job_name.capitalize()}: {diffusion_count} operations"
                    )
    ...
    if all(len(combo) == 1 for combo in machine_combinations):
        ...
        if mlp_debug:
            ...
            for i, mlp_indices in enumerate(mlp_indices_list):
                ...
                print(
                    f"  {mlp_job_names[i].capitalize()}: CPU_P={mlp_cpu_p}, CPU_E={mlp_cpu_e}"
                )
        else:
            if not no_depth_network and first_network_indices:
                first_network_cpu_p = sum(
                    1 for i in first_network_indices if np.argmax(alpha[i]) == 0
                )
                first_network_cpu_e = sum(
                    1 for i in first_network_indices if np.argmax(alpha[i]) == 1
                )
                print(
                    f"  {first_network_job_name.capitalize()}: CPU_P={first_network_cpu_p}, CPU_E={first_network_cpu_e}"
                )
            if dronet_indices:
                dronet_cpu_p = sum(
                    1 for i in dronet_indices if np.argmax(alpha[i]) == 0
                )
                dronet_cpu_e = sum(
                    1 for i in dronet_indices if np.argmax(alpha[i]) == 1
                )
                print(
                    f"  {dronet_job_name.capitalize()}: CPU_P={dronet_cpu_p}, CPU_E={dronet_cpu_e}"
                )
            if not use_mobilenet and not use_diffusion:
                for i, mlp_indices in enumerate(mlp_indices_list):
                    mlp_cpu_p = sum(
                        1 for idx in mlp_indices if np.argmax(alpha[idx]) == 0
                    )
                    mlp_cpu_e = sum(
                        1 for idx in mlp_indices if np.argmax(alpha[idx]) == 1
                    )
                    print(
                        f"  {mlp_job_names[i].capitalize()}: CPU_P={mlp_cpu_p}, CPU_E={mlp_cpu_e}"
                    )
                ...
                if use_mobilenet:
                    mobilenet_cpu_p = sum(
                        1 for idx in mobilenet_indices if np.argmax(alpha[idx]) == 0
                    )
                    mobilenet_cpu_e = sum(
                        1 for idx in mobilenet_indices if np.argmax(alpha[idx]) == 1
                    )
                    print(
                        f"  {mobilenet_job_name.capitalize()}: CPU_P={mobilenet_cpu_p}, CPU_E={mobilenet_cpu_e}"
                    )
                if use_diffusion:
                    diffusion_cpu_p = sum(
                        1 for idx in diffusion_indices if np.argmax(alpha[idx]) == 0
                    )
                    diffusion_cpu_e = sum(
                        1 for idx in diffusion_indices if np.argmax(alpha[idx]) == 1
                    )
                    print(
                        f"  {diffusion_job_name.capitalize()}: CPU_P={diffusion_cpu_p}, CPU_E={diffusion_cpu_e}"
                    )
    ...
    is_valid, validation_results = validate_schedule(
        ...
        profiled_times_e=all_profiled_times_e if all_profiled_times_e else None,
        profiled_times_by_network=(
            profiled_times_by_network if profiled_times_by_network else None
        ),
...
if __name__ == "__main__":
    ...
    if args.mlp_debug and (args.use_mobilenet or args.diffusion):
        parser.error(
            "--mlp-debug is mutually exclusive with --use-mobilenet and --diffusion"
        )
