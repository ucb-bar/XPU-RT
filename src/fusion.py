...
class FusedOperation(Operation):
    ...
    def __init__(
        self,
        original_operations: List[Operation],
        ...
        self.original_operations = (
            original_operations  # List of original operations that were fused
        )
        self.fused_operation_ids = [
            op.operation_id for op in original_operations if op.operation_id is not None
        ]
...
def get_min_duration(
    operation: Operation, machine_combinations: List[List[str]], machines: List[str]
) -> float:
    """
...
def would_create_cycle(
    group: Set[int], workload: Workload, existing_fusion_groups: List[Set[int]]
) -> bool:
    """
...
def fuse_operations(
...
) -> Tuple[Workload, Dict[int, List[int]]]:
    """
    ...
    def find_linear_chain(start_idx: int) -> List[int]:
        ...
        while (
            ...
            and current_idx in below_threshold
    ...
    for chain in fusion_groups:
        ...
        for machine_idx in range(len(machines)):
            total_dur = sum(
                op.processing_times[machine_idx]
...
def expand_schedule(
    ...
    if t_fused is None or alpha_fused is None:
        raise ValueError(
            "Cannot expand schedule: fused schedule is None (optimization failed)"
        )
        ...
        if isinstance(fused_op, FusedOperation):
            ...
            for i, orig_idx in enumerate(original_indices):
                if orig_idx >= num_original:
                    raise ValueError(
                        f"Invalid original index {orig_idx} (max: {num_original-1})"
                    )
    ...
    if len(scheduled_ops) != num_original:
        ...
        raise ValueError(
            f"Not all operations were scheduled! Missing {len(missing)} operations: {sorted(missing)[:10]}..."
        )
...
def print_fusion_report(
    ...
    try:
        ...
        print(
            f"Reduction: {len(original_workload.operations) - len(fused_workload.operations)} operations fused"
        )
        ...
        for fused_idx, original_indices in fusion_groups:
            ...
            print(
                f"  Contains {len(original_indices)} original operations: {original_indices}"
            )
            ...
            for pred in preds:
                ...
                if pred_fused_idx is not None:
                    ...
                    print(
                        f"    - Fused op {pred_fused_idx} (original ops: {pred_orig_indices})"
                    )
                else:
                    print(
                        f"    - {pred.operation_name if hasattr(pred, 'operation_name') else 'Unknown'}"
                    )
                ...
                for j, orig_idx_j in enumerate(original_indices):
                    if i != j:
                        ...
                        if orig_op_j in orig_op_i.get_predecessors():
                            print(
                                f"    - Original op {orig_idx_i} depends on original op {orig_idx_j} (within fusion group)"
                            )
        ...
        for fused_idx, fused_op in enumerate(fused_workload.operations):
            ...
            if isinstance(fused_op, FusedOperation):
                ...
                for orig_idx in orig_indices:
                    orig_op = original_workload.operations[orig_idx]
                    name = (
                        orig_op.operation_name
                        or orig_op.operation_id
                        or f"op{orig_idx}"
                    )
                    names.append(name)
        ...
        def find_cycles_dfs(
            node_idx, path, visited, rec_stack, graph, cycles, names_map
        ):
            visited[node_idx] = True
            rec_stack[node_idx] = True
            ...
            for neighbor_idx in graph.get(node_idx, []):
                if neighbor_idx in path:
                    ...
                    find_cycles_dfs(
                        neighbor_idx, path, visited, rec_stack, graph, cycles, names_map
                    )
        ...
        if cycles:
            print(
                f"  WARNING: {len(cycles)} cycle(s) detected in fused dependency graph!"
            )
            print("\n  CYCLE DETAILS:")
            for cycle_idx, cycle in enumerate(cycles, 1):
                ...
                for i, fused_idx in enumerate(cycle[:-1]):  # Don't repeat the last node
                    ...
                    print(
                        f"      FusedOp{fused_idx} ({', '.join(names)}) -> FusedOp{next_idx} ({', '.join(next_names)})"
                    )
                ...
                print(
                    f"      FusedOp{last_idx} ({', '.join(last_names)}) -> FusedOp{first_idx} ({', '.join(first_names)}) [CLOSES CYCLE]"
                )
        else:
        ...
        entry_points = [
            i
            for i, op in enumerate(fused_workload.operations)
            if not op.get_predecessors()
        ]
