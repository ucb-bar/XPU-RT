...
static void print_usage(const char* argv0) {
  fprintf(stderr,
          ...
          "[report_every] [warmup_graph_iters] [csv_out]\n"
          "\n"
          "Defaults:\n"
          "  driver    = local-task\n"
          "  graph_iters    = 1\n"
...
int main(int argc, char** argv) {
  ...
  const char* csv_out = (argc >= 8) ? argv[7] : "dispatch_trace.csv";
  ...
  // CSV trace (timed section only).
  csv = fopen(csv_out, "wb");
  if (!csv) {
    status = iree_make_status(IREE_STATUS_NOT_FOUND,
                              "failed to open csv_out for write: %s", csv_out);
    goto cleanup;
  }
  // Large buffer to reduce IO overhead.
  setvbuf(csv, NULL, _IOFBF, 1 << 20);
  fprintf(csv,
          "graph_iter,dispatch_order,dispatch_key,dispatch_id,start_ns,"
          "duration_ns,start_ms,duration_ms\n");
  ...
  for (int gi = 0; gi < graph_iters; ++gi) {
    for (size_t oi = 0; oi < order_count; ++oi) {
      ...
      const uint64_t start_rel_ns = t0 - run_start_ns;
      const uint64_t dur_ns = t1 - t0;
      fprintf(csv,
              "%d,%" PRIu64 ",%s,%d,%" PRIu64 ",%" PRIu64 ",%.6f,%.6f\n", gi,
              (uint64_t)oi, n->key ? n->key : "", n->id, start_rel_ns, dur_ns,
              (double)start_rel_ns / 1000000.0, (double)dur_ns / 1000000.0);
      if (!iree_status_is_ok(status)) goto cleanup;
    }
  ...
  if (csv) {
    fflush(csv);
    fprintf(stdout, "CSV trace written to %s\n", csv_out);
  }
  fprintf(stdout, "Done.\n");
  fflush(stdout);
