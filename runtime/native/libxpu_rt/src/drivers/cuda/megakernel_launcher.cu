...
#include "../../../include/xpu_rt/xpu_rt.h"
...
cg_rt_status_t cg_rt_cuda_megakernel_launch(
    ...
    if (config->shared_mem_bytes > 49152) {
        ...
        if (rc_attr != CUDA_SUCCESS) {
            ...
            fprintf(stderr,
                "xpu_rt: cuFuncSetAttribute(MAX_DYNAMIC_SHARED_SIZE_BYTES=%u) failed: %s\n",
    ...
    if (rc != CUDA_SUCCESS) {
        ...
        fprintf(stderr,
            "xpu_rt: cuLaunchKernelEx failed: %s "
    ...
    if (rc_sync != cudaSuccess) {
        fprintf(stderr,
            "xpu_rt: cudaDeviceSynchronize after megakernel launch failed: %s\n",
