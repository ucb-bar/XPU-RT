...
XPU-RT currently has two hardware-description surfaces that matter to users:
...
`xpu_rt.device()` currently expects a richer targetgen hardware spec. The public example at `examples/hardware_specs/gpu_simt_demo.yaml` mirrors the style exercised in `tests/targetgen/exemplars/`.
...
2. Get `xpu_rt.device()` working with your edited spec.
3. Inspect the generated artifacts in the output directory.
4. Move to `xpu_rt.compile_model()` once your target description is stable.

## What Is Not Ready Yet

The `xpu-rt scaffold-target` CLI command is documented, but it is still a stub. For now, use the Python API and targetgen modules directly if you need a real bring-up workflow.
