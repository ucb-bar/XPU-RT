...
| Device function / graph function | `func.func` with `xpu_rt.event.device_fn` / `xpu_rt.event.graph_fn` attributes |
| Data-dependent `topk` / `exp_indptr` | `xpu_rt.event.update` / `xpu_rt.event.trigger` |
...
The `xpu_rt.event` dialect and the three passes are the only
...
FX → xDSL  (bridge_fx_graph)
    │
    ▼
xpu_rt.event graph construction
    │
    ▼
...
Static scheduling is deterministic (`xpu_rt.event.device_fn`
...
- **Dialect**: `python/xpu_rt/ir/event/` — ops, types, dialect
  registration.
- **Scheduling passes**: `python/xpu_rt/ir/payload/passes/`
  (`megakernel_static_schedule`, `megakernel_dynamic_schedule`).
- **Runtime lowering**:
  `python/xpu_rt/ir/payload/passes/rewrites/lower_event_tensor_to_atomic.py`.
- **Megakernel emission**: `python/xpu_rt/ir/tile/lower_megakernel.py`
...
- **XPU-RT does not** take a TVM dependency; every piece (dialect,
