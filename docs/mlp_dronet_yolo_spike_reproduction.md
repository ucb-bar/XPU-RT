...
pytorch.org during the from-scratch run. All of it is now installed by one
script, `scripts/install_xpurt_deps.sh` (run from the top-level repo, with
the `zephyr` env already active) — the dependency-management cleanup
below folded what used to be a hand-copied command sequence here into that
one place, since scattering "what xpurt needs on top of a standalone
zephyr-chipyard-sw install" across doc prose was exactly what made Bugs
13-17 easy to introduce and hard to keep in sync:

```bash
pip install -r zephyr_ws/zephyr/scripts/requirements-base.txt  # west's own full base requirements
bash scripts/install_xpurt_deps.sh
```

What that script does, and why (kept here for the "why", not as a separate
...
- **`spike` itself.** Despite the custom-looking version string
  ...
  hashes from setuptools_scm), this exact version IS a normal, public PyPI
  package (github.com/liuyu81/pyspike, Apache-2.0) with a prebuilt
  manylinux wheel — no local compilation needed. (Gemmini/Saturn RoCC
  support is NOT compiled into this wheel — it comes from `libgemmini.so`,
  a runtime `--extlib`-style plugin generated by modelblaster's own
  tooling and already committed under
  `modelblaster/cores/gemmini/include/per_config/`, unrelated to the spike
  package itself.)
- **modelblaster's own deps** (torch, numpy, pillow, `ultralytics>=8.4.55`
  — needed for yolov8_nano's extraction step — via modelblaster's own
  `pyproject.toml`, installed in editable mode).
- **`libgl1` (system package, Bug 16).** `ultralytics` pulls in
  `opencv-python`, which dynamically links `libGL.so.1` at import time —
  not present on a bare/headless install (confirmed missing on a plain
...
- **xpu-rt's own scheduler deps** (numpy/scipy/matplotlib/pandas — not
  previously declared anywhere; satisfied only by accident via whatever
  env happened to already have them) via this repo's own `pyproject.toml`.
