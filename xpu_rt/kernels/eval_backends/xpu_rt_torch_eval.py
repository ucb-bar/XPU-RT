"""XPU-RT-native autocomp ``EvalBackend`` that doesn't shell out to
KernelBench.

Autocomp's stock ``KBEvalBackend`` requires a full KernelBench
checkout + a reference ``Model`` file pulled by problem id. For
XPU-RT-driven contracts we already have the reference module in
process (typically a ``torch.nn.functional`` baseline), so we run
the candidate ``ModelNew`` against a XPU-RT-provided reference
without any external script.

Returned per-candidate dict matches what
``xpu_rt/third_party/autocomp/autocomp/search/search.py`` reads::
...
try:
    import torch
except ImportError:  # pragma: no cover - torch always present in XPU-RT venv
    torch = None  # type: ignore[assignment]


class XpuRtTorchEvalBackend:
    """Autocomp-compatible eval backend powered by torch.

    Drop-in replacement for ``KBEvalBackend`` for XPU-RT contracts
    ...
    def __init__(
        ...
        *,
        ...
        if torch is None:
            raise RuntimeError("torch is required for XpuRtTorchEvalBackend")
    ...
    def _run(self, code_str: str) -> dict[str, Any]:
        ...
        tmp_dir = tempfile.mkdtemp(prefix="xpu_rt_eval_cand_")
        mod_name = f"xpu_rt_cand_{uuid.uuid4().hex[:12]}"
