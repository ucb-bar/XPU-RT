"""``XpuRtOptions`` -- single dataclass of compilation knobs.
...
Waves 2+ passes accept a ``XpuRtOptions`` argument and read their own
switch from it.
...
- Reproducibility: the entire compilation of a model is characterized
  by ``(model, inputs, XpuRtOptions)``. If those three hash to a
...
class XpuRtOptions:
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> XpuRtOptions:
    ...
    def replace(self, **changes: Any) -> XpuRtOptions:
        ...
        return XpuRtOptions(**current)


# --- preset profiles ---------------------------------------------------


def cuda_a100_defaults() -> XpuRtOptions:
...
def cuda_h100_defaults() -> XpuRtOptions:
...
def npu_fp8_defaults() -> XpuRtOptions:
