"""Install (merge) the XPU-RT MCP server into a client config file.
...
* The ``mcpServers.xpu_rt`` key is set to
  :func:`xpu_rt.mcp.config.mcp_server_entry`. If the key already exists
...
from xpu_rt.mcp.config import SERVER_NAME, mcp_server_entry
...
def install_mcp_server(
    *,
    ...
    """Merge the XPU-RT MCP server into a client config file.
    ...
    Args:
        ...
        force: Overwrite an existing ``mcpServers.xpu_rt`` entry that
