...
* The resolved :class:`~xpu_rt.api.XpuRtDevice` (from ``open_target``).
* The loaded model + sample inputs (from ``load_model``).
* The live :class:`~xpu_rt.agent.llm_driver.LLMDrivenCompiler`
...
restarting ``xpu-rt-mcp`` drops all sessions. Cross-session knowledge
lives in ``~/.xpu_rt/transcripts`` + ``~/.xpu_rt/memory.sqlite`` via
the existing :class:`~xpu_rt.llm.recorder.ToolCallRecorder` +
...
from xpu_rt.api import XpuRtDevice, CompiledModel
from xpu_rt.llm.base import XpuRtLLMProtocol
...
class McpSession:
    ...
    kernel_cache: Any = None  # set lazily; see xpu_rt.mcp.tools.kernel._kernel_cache
    ...
    # In-session bench cache (W7.1) — see xpu_rt.mcp.tools.bench._bench_cache
    bench_cache: Any = None
    # Per-session optimisation progress tracker (W7.3)
    optim_progress: Any = None
    # In-session refinement cache (W8.2) — see xpu_rt.mcp.tools.refinement
    refinement_cache: Any = None
    # In-session autotune cache (W8.3) — see xpu_rt.mcp.tools.autotune
    autotune_cache: Any = None
    # In-session decision-site registry. Stage plugins enqueue sites
    # here via :mod:`xpu_rt.agent.decisions`; MCP tools read/write
    # them via :mod:`xpu_rt.mcp.tools.decisions`.
    ...
    # flag ``XPU_RT_STRICT_PHASE_GATING=1``.
    ...
    def require_device(self) -> XpuRtDevice:
...
class SessionManager:
    ...
    def __init__(self, scratch_root: Path | None = None) -> None:
        ...
        self._scratch_root = scratch_root or Path(tempfile.gettempdir()) / "xpu-rt-mcp"
