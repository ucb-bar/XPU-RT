...
- :mod:`xpu_rt.runtime.verification.plan_refinement` — every plan op
  corresponds to one launch in the emit, in the declared order.
- :mod:`xpu_rt.runtime.verification.abi_conformance` — the emit
  calls only ``cg_rt_*`` and ``xpu_rt_kernel_*`` externs; nothing
  else (no ``cudaMalloc``, no ``hipLaunchKernel``, no ``vkCmd*``).
- :mod:`xpu_rt.runtime.verification.resource_budget` — static
...
from :mod:`xpu_rt.runtime.errors` on failure. Callers compose the
...
from xpu_rt.runtime.verification.runner import (
