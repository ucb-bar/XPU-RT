...
:class:`~xpu_rt.runtime.baremetal.emitter.BaremetalEmitter` scaffold
...
_AGENT_DECISION_ATTRS: tuple[str, ...] = (
    "xpu_rt.region_id",
    "xpu_rt._pattern_hint",  # op-family role (matmul/softmax/rmsnorm/...) from import_fx
    "xpu_rt.fused_into",
    "xpu_rt.fusion_kind",
    "xpu_rt.tile_sizes_str",
    "xpu_rt.megakernel",
    "xpu_rt.device",
)
...
def _agent_trail(op: Operation) -> str:
    """Build a short ``key=value`` trail of xpu_rt.* attrs for inline comments."""
    parts: list[str] = []
    for k in _AGENT_DECISION_ATTRS:
        ...
        if not text:
            continue
        # Strip the xpu_rt. prefix in the comment for legibility.
        short = k.removeprefix("xpu_rt.")
...
def emit_npu_driver_extension_h(
    ...
    *,
    ...
    lines = [
        ...
        "",
        f"/* XPU-RT-emitted npu helpers for {model_name}. */",
        "#include <stdint.h>",
        "#include <stddef.h>",
        "",
 ...
 * Auto-generated by XPU-RT. Replace with HVX intrinsic versions
