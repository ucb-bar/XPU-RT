"""Autotuning harness over ``XpuRtOptions``.
...
class OptionsAxis:
    """One search dimension over a single XpuRtOptions field.
    ...
    def __post_init__(self) -> None:
        ...
        # Validate the field exists on XpuRtOptions.
        import dataclasses

        fields = {f.name for f in dataclasses.fields(XpuRtOptions)}
        if self.field_name not in fields:
            raise ValueError(f"{self.field_name!r} is not a XpuRtOptions field; known: {sorted(fields)[:10]}...")
...
class Autotuner:
    """Search over ``XpuRtOptions`` configurations."""
    ...
    def _enumerate(self) -> Iterable[XpuRtOptions]:
