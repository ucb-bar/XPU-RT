...
``xpu_rt.targets.registry`` calls into your adapters when a user
...
# ``runtime.py``, ``cost.py``.
from xpu_rt.targets._template.body_emitter import TemplateBodyEmitter
from xpu_rt.targets._template.cost import TemplateCostModel
from xpu_rt.targets._template.probe import TemplateProbe
from xpu_rt.targets._template.runtime import TemplateRuntime


def _register_template() -> None:
    """When this template is copied into a real target package,
    rename to ``_register_<your_arch>`` and call
    :func:`xpu_rt.targets.registry.register_target` with concrete
    metadata."""
