...
from xpu_rt.api import XpuRtDevice, CompiledModel, compile_model
...
def _resolve_target(target: str | Path | XpuRtDevice) -> XpuRtDevice:
    if isinstance(target, XpuRtDevice):
        return target
...
def _resolve_llm(
    llm: str | XpuRtLLMProtocol,
) -> tuple[XpuRtLLMProtocol, str]:
    ...
    # Duck-typed client already — accept if it looks like a XpuRtLLMProtocol.
...
def compile_with_llm(
    model: str | Path | nn.Module,
    target: str | Path | XpuRtDevice,
    *,
    llm: str | XpuRtLLMProtocol = "gemini",
    ...
    Args:
        ...
        llm: Provider name (``"gemini" | "openai" | "anthropic" |
            ...
            :class:`XpuRtLLMProtocol`. Resolved through
