...
def main():
    ...
    # SWEEP HARNESS: cameras kept at FULL resolution so rendering (and thus the front-camera model
    # input) is bit-identical to record_sensor_demo.py. We only skip the matplotlib compositor + video
    # encode (a pure read/CPU step that cannot affect physics) and run all episodes. Validated to
    # reproduce the recorder's per-seed outcomes exactly.
    ...
    _sweep_results = []           # SWEEP: (seed, outcome, gates, steps) per episode
    ...
    for ep in range(args_cli.episodes):
        ...
        class _NullWriter:                       # SWEEP: no video; keep API surface
        ...
        for t in range(args_cli.max_steps):
            ...
            _drive_chase()   # keep identical to recorder (cosmetic pose set; kept for equivalence)
            ...
            fires = {"CTRL": (t % G_CTRL == 0), "NAV": True, "YOLO": yolo_fire,
                     "CAM": (t % G_FPV == 0)}
            # SWEEP: skip the (expensive matplotlib) compositor + video write entirely
            _ = (banner, fires)
    ...
    if getattr(args_cli, "sweep_csv", None):
        import csv as _csv
        _cdt_ms = control_dt * 1000.0
        _eff_hz = 1000.0 / (_cdt_ms * _ctrl_refresh)
        _new = not os.path.exists(args_cli.sweep_csv)
        with open(args_cli.sweep_csv, "a", newline="") as _cf:
            _w = _csv.writer(_cf)
            if _new:
