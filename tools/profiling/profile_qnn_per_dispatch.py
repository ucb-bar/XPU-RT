...
# --------------------------------------------------------------------------- #
# Programmatic per-dispatch lookup — used by the agentic CLI (and the
# MCP tools' --dry-run path) to synthesise a profiled_manifest cell
# from the cached cost table without actually running on the board.
# --------------------------------------------------------------------------- #
...
def profile_one_dispatch(
    ...
    target: str,
    *,
    cost_table_path: pathlib.Path,
    op_key_hint: str | None = None,
) -> dict:
    """Return a profiled_manifest cell for ``(dispatch_id, target)``.

    Looks the dispatch up in the cached CostTable; when no exact row
    matches, returns ``{"mean_us": None, "source": "missing"}`` so the
    caller can decide whether to escalate to a real on-board run.

    ``op_key_hint`` lets callers pass the canonical CostTable key
    directly when the dispatch name doesn't carry enough info to
    recover it.
    """
    ...
    # Strategy 1: explicit op_key_hint + backend.
    candidates: list[str] = []
    if op_key_hint:
        candidates.append(f"{op_key_hint}::{backend}::0")
        candidates.append(f"{op_key_hint}::{backend}::1")
    # Strategy 2: substring match on dispatch_name in the cost table rows.
...
def synthesise_profiled_manifest(
    *,
    ...
    Used by the agentic loop's ``--dry-run`` so the rest of the
    pipeline can proceed without an actual board. The output schema
    matches what merlin's profile_dispatch_matrix.py emits:

        {"dispatches": {<name>: {<target>: {"mean_us": x, ...}}}}
    """
