...
# --------------------------------------------------------------------------- #
# Programmatic round entrypoint — used by the MCP tools and the
# agentic CLI to step the loop one round at a time. The existing
# ``main()`` still drives the full multi-round loop for back-compat;
# this helper does NOT mutate global state and may be called from
# in-process callers (no argparse plumbing required).
# --------------------------------------------------------------------------- #


@dataclasses.dataclass
class RoundOutcome:
    """Per-round artefacts produced by :func:`run_single_round`."""

    round_dir: pathlib.Path
    matrix: pathlib.Path
    profiled_manifest: pathlib.Path
    schedule: pathlib.Path
    next_source: pathlib.Path
...
def run_single_round(
    ...
    *,
    k: int,
    cur_source: pathlib.Path,
    prev_schedule: pathlib.Path | None = None,
) -> RoundOutcome:
    """Execute one round of the heterogeneous loop in-process.

    Mirrors the body of ``main()``'s round loop but: (a) returns a
    typed outcome object, (b) does not write the convergence symlink
    or invoke ``run_on_board_flow.py`` again (the caller owns those
    side-effects when it stops iterating), (c) re-uses the existing
    step functions so the validated behaviour is identical.
    """
    ...
    stable = placements_match(prev_schedule, sched)
    next_source = cur_source if stable else step_apply_requant(
        cfg, cur_source, sched, rd,
    )
    makespan = 0.0
    try:
        makespan = float(json.loads(sched.read_text()).get("makespan_us", 0.0))
    except (OSError, json.JSONDecodeError, ValueError):
        pass
    return RoundOutcome(
        round_dir=rd,
        matrix=matrix,
        profiled_manifest=prof,
        schedule=sched,
...
def materialise_source_from_onnx(
    onnx_path: pathlib.Path,
    out_mlir: pathlib.Path,
    *,
    opset: int = 18,
    workload_id: str | None = None,
    allow_stub: bool = True,
) -> pathlib.Path:
    """Bridge ONNX → payload MLIR via xpu_rt.targets.backends.qnn.onnx_bridge.

    The heterogeneous loop expects ``--source <mlir>``; this helper
    lets callers pass an ONNX instead by routing through the bridge.
    """
    from xpu_rt.targets.backends.qnn.onnx_bridge import onnx_to_payload_mlir
    ...
    return result.mlir_path


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--source", type=pathlib.Path, required=False, default=None,
                   help="Pre-built MLIR source. Either this or --source-onnx "
                        "must be supplied.")
    p.add_argument("--source-onnx", type=pathlib.Path, default=None,
                   help="ONNX model; converted to payload MLIR via "
                        "xpu_rt.targets.backends.qnn.onnx_bridge before "
                        "round 0.")
    ...
    if args.source is None and args.source_onnx is None:
        print("error: one of --source / --source-onnx is required",
              file=sys.stderr)
        return 2
    if args.source is None:
        bridge_out = args.out_dir.resolve() / "payload_from_onnx.mlir"
        bridge_out.parent.mkdir(parents=True, exist_ok=True)
        args.source = materialise_source_from_onnx(
            args.source_onnx.resolve(), bridge_out,
            workload_id=args.source_onnx.stem,
        )
        print(f"[onnx-bridge] {args.source_onnx} -> {args.source}")
